...
function AddReunion() {
  ...
  const handleAddReunion = async () => {
    const res = await axios.post(
      "http://localhost:3000/events/addreunion",
      formdata
    );
    navigate("/events/reunions");
  };
          ...
          <div className="flex justify-center">
            <div className="bg-[#D9D9D9] m-5 p-3 rounded-lg w-[50%] flex-col justify-center max-lg:w-full border-2 border-black">
              <div className="bg-[#F5A7A7] text-3xl p-3 rounded-lg text-center mb-5 flex justify-between max-sm:flex-col">
                <div>Reunion Name:</div>
                <div>
                  <input
                    type="text"
                    name="reunionName"
                    placeholder="Reunion Name"
                    onChange={(e) => {
                      setFormdata({ ...formdata, reunionName: e.target.value });
                    }}
              ...
              <div className="bg-[#F5A7A7] text-3xl p-3 rounded-lg text-center mb-5 flex-col justify-center">
                <div>Reunion details:</div>
                <div>
                  <textarea
                    type="text"
                    name="reunionDetails"
                    placeholder="Reunion Details"
                    rows="10"
                    style={{ width: "100%" }}
                    onChange={(e) => {
                      setFormdata({
                        ...formdata,
                        reunionDetails: e.target.value,
                      });
                    }}
              ...
              <div className="bg-[#F5A7A7] text-3xl p-3 rounded-lg text-center mb-5 flex justify-between max-sm:flex-col">
                <div>Reunion PhotoURL:</div>
                <div>
                  <input
                    type="text"
                    name="reunionPhotourl"
                    placeholder="Reunion Image Link"
                    onChange={(e) => {
                      setFormdata({
                        ...formdata,
                        reunionimgurl: e.target.value,
                      });
                    }}
                    className="rounded-lg border-2 border-black ml-3 text-center font-serif"></input>
                </div>
